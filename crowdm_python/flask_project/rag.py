...
def getmessage_abno(situ, time, h_yn, location, video_index, eventFeatures):
...
# 임베딩 데이터와 쿼리 임베딩 비교
    ...
    prompt = prompt.format(time, location, situ, eventFeatures, text1=passages[0],text2=passages[1],text3=passages[2],text4=passages[3], text5=passages[4], query=query)
...
답변은 아래의 예시 형식으로 해주세요.  특히 반드시 location의 건물, 장소 특성(eventFeatures)을 고려해주세요.
"""},
...
def getmessage_abyes(situ, time, h_yn, location, video_index, eventFeatures):
# 쿼리 텍스트 임베딩 생성 및 유사도 검색 수행
...
# 임베딩 데이터와 쿼리 임베딩 비교
    ...
    for item in embeddings_data:
        ...
        prompt = """                                                                                                                                                                                                                                                            너는 군중 관리 전문가이며, 주어진 상황에 대한 인파 관리 방안을 제시해야 합니다. 아래 정보와 문서를 참고하여 질문에 답변해 주세요. 모르는 내용이 있다면 모른다고 솔직하게 답해 주세요. 답변은 한국어로 작성해 주세요.
                    ...
                    예시:
                    [질문]: 2022년 12월 25일, 휴일, 상왕십리에서 폭행 이상행동이 발생했습니다. 대응 방법 및 피해가 커지지 않도록 하는 방법을 알려 주세요.

                    [답변]:
                    1. 상황 설명
                    2022년 12월 25일, 평일인 상왕십리에서 폭행 이상행동이 발생했습니다.

                    2. 시간과 장소의 특성 기술
                    - 상왕십리는 도심 지역에 위치하며 대중교통이 발달한 곳으로, 사람들의 이동이 많은 특성을 가지고 있습니다. 폭행 이상행동이 발생할 경우 피해자가 발생할 위험이 증가할 수 있습니다.

                    3. 대응 방안
                    - 대응 1: 추가 경찰 및 보안 요원을 배치하여 안전을 유지하고 이상행동자를 진정시킵니다.
                    - 대응 2: 출입구 및 주요 통로 주변에 안내원을 배치하여 방향 지시 및 안전을 도우며, 상황 발생 지역의 통행을 원활하게 합니다.
                    - 대응 3: 상황 발생 지역 주변에 응급 대피 및 구조팀을 배치하여 긴급 상황에 신속히 대처할 수 있도록 합니다.
                    - 대응 4: 폭행 가해자와 다른 사람들을 격리시키십시오. 방송을 활용하는 것도 좋습니다.
    ...
    response = client.chat.completions.create(
        ...
        답변은 아래의 예시 형식으로 해주세요. query에서 time, location, ab_type을 추출하여 응답에 활용하세요. 특히 반드시 location의 건물, 장소 특성(eventFeatures)을 고려해주세요.
        """},
            {"role": "user", "content": prompt}
        ]
        )
...
def guide(situ, time, h_yn, location, eventFeatures) :
    ...
    if '혼잡' in situ:
        ...
        result=getmessage_abno(situ, time, h_yn, location, video_index, eventFeatures)
        
    else:
        print('===============================================================================')
        print('abyes 발동')
        print('video_index :', video_index)
        result=getmessage_abyes(situ, time, h_yn, location, video_index, eventFeatures)
